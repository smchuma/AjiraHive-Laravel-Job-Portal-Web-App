<form class="w-full max-w-lg mt-10" action="{{route('store.employer')}}" method="post" >
    @csrf
      ...
      <div class="flex flex-wrap -mx-3 mb-6">
        <div class="w-full px-3">
          ...
          <input class=" placeholder:text-gray-500 appearance-none block w-full bg-gray-100 {{$errors->has('full_name')  ? 'border-red-500' : ''}}  text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-password" type="text" name="full_name" placeholder="Enter Name">
          @if ($errors->has('full_name'))     
          <p class="text-red-500 text-xs italic mt-2 mb-3">{{$errors->first('full_name')}}</p>          
          @endif
          <label class="block tracking-wide text-black text-xs font-bold mb-2" for="grid-password">
            Email
          </label>
